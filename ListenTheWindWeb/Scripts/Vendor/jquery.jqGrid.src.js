    ...
    $.fn.jqGrid = function (pin) {
        ...
        return this.each(function () {
            ...
            var grid = {
                ...
                dragEnd: function () {
                    ...
                    if (this.resizing) {
                        //Peter Sun change, for drag
                        var idx = this.resizing.idx;
                        var headObj = $("#" + ts.p.id + "_" + ts.p.colModel[idx].name);
                        var oldWidth = headObj[0].scrollWidth;
                        var nw = this.headers[idx].newWidth || oldWidth;
                        nw = parseInt(nw, 10);
                        this.resizing = false;
		...
		populate = function (npage) {
		    if (!ts.grid.hDiv.loading) {
		        ...
		        if (pN.rows !== null) { prm[pN.rows] = ts.p.rowNum; } if (pN.page !== null) { prm[pN.page] = intNum(ts.p.page); }
		...
		getOffset = function (iCol) {
		    ...
		    for (i = 0; i <= iCol; i++) {
		        if (ts.p.colModel[i].hidden === false) {
		            //Peter Sun Changed
		            var headObj = $("#" + ts.p.id + "_" + ts.p.colModel[i].name);
		            ret[0] += headObj[0].scrollWidth + brd1;
		        }
		    }
		...
		td, ptr, tbody, imgs, iac = "", idc = "";
            ...
            for (i = 0; i < this.p.colNames.length; i++) {
                ...
                if (!ts.p.colModel[i].width) {
                    ts.p.colModel[i].width = 20;
                    //Removed by Peter Sun 
		...
		firstr = "<tr class='jqgfirstrow' role='row' style='height:auto'>";
            ...
            $("th", thr).each(function (j) {
                ...
                firstr += "<td role='gridcell' id = firstRowTd_" + ts.p.colModel[j].name + " style='height:0px;width:" + w + "px;" + hdcol + "'></td>";
		...
		.append("<span class='ui-icon ui-icon-circle-triangle-n'></span>").css((dir == "rtl" ? "left" : "right"), "0px") : "";
            /*Added by Peter Sun*/
            var fullTextSearch = "";
            if (ts.p.showview == true) {
                fullTextSearch += "<select id = '" + ts.p.id + "_view'></select>";
            }
            if (ts.p.showfulltextsearch == true) {
                fullTextSearch += "<div style='float:right'><input type='search'  id ='" + ts.p.id + "_Search_Box' /><input type = 'button' id = '" + ts.p.id + "_search_button' onclick = '" + ts.p.sourcefile + "." + ts.p.id + "_search()' value = 'Find' /></div>"
            }
            $(grid.cDiv).append(arf).append("<span class='ui-jqgrid-title" + (dir == "rtl" ? "-rtl" : "") + "'>" + ts.p.caption + "</span>" + fullTextSearch)

            //</span><input type='search' id = '" + ts.p.id + "_Search_Box' /><input type = 'button' id = '" + ts.p.id + "_search_button' onclick = '" + ts.p.id + "_search()' value = 'Find' />
...
(function ($) {
    ...
    $.fn.jqFilter = function (arg) {
        ...
        var p = $.extend(true, {
		    ...
		    { "name": "bt", "description": "between", "operator": "?" },
		    { "name": "ert", "description": "earlier than", "operator": "?" },
		    { "name": "ere", "description": "earlier than or equal", "operator": "?" },
		    { "name": "ltt", "description": "later than", "operator": "?" },
		    { "name": "lte", "description": "later than or equal", "operator": "?" }
		],
            numopts: ['eq', 'ne', 'lt', 'le', 'gt', 'ge', 'nu', 'nn', 'bt'],
            stropts: ['cn', 'eq', 'ne', 'bw', 'bn', 'ew', 'en', 'nc', 'nu', 'nn'],
            dateopts: ['bt', 'ert', 'ere', 'ltt', 'lte'],
            ...
            */
			...
			i, op, trpar, cm, str = "", selected;
                ...
                ruleFieldSelect.bind('change', function () {
                    ...
                    // The following sentence is to make the two elements with different Ids.
                    cm.searchoptions.id += 120;
                    var elm2 = $.jgrid.createEl(cm.inputtype, cm.searchoptions, "", true, that.p.ajaxSelectOptions, true);
                    $(elm2).addClass("input-elm");
                    var hyp = $('<span class="jqg-hyphen">-</span>');
                    //that.createElement(rule, "");

                    if (cm.searchoptions.sopt) { op = cm.searchoptions.sopt; }
                    else if (that.p.sopt) { op = that.p.sopt; }
                    else if (cm.searchtype === 'string') { op = that.p.stropts; }
                    else if (cm.searchtype === 'date') {
                        op = that.p.dateopts;
                    }
                    ...
                    //$(".data", trpar).append("<select class='selectopts'></select>");

                    $('.data', trpar).append(hyp);
                    $(".data", trpar).append(elm2);
                    if ($('.selectopts', trpar).val() == 'bt') {
                        hyp.css('display', '');
                        $(elm2).css('display', '');
                    } else {
                        hyp.css('display', 'none');
                        $(elm2).css('display', 'none');
                    }
                    $(".input-elm", trpar).bind('change', function () {
                        rule.data = $(elm).val() + ($(elm2).css('display') == 'none' ? '' : ('&' + $(elm2).val()));
                        that.onchange(); // signals that the filter has changed
                    });
                ...
                var ruleDataInput2 = $.jgrid.createEl(cm.inputtype, cm.searchoptions, rule.data, true, that.p.ajaxSelectOptions, true);
                var ruleDataInputHyphen = $('<span class="jqg-hyphen">-</span>');
                ...
                ruleOperatorSelect.bind('change', function () {
                    ...
                    var rd2 = $(".input-elm", trpar)[1];
                    var hyp = $(".jqg-hyphen", trpar)[0];
                    ...
                    rule.data = $(rd).val() + ($(rd2).css('display') == 'none' ? '' : ('&' + $(rd2).val()));
                    that.onchange();  // signals that the filter has changed
                });
                ...
                ruleDataTd.append(ruleDataInputHyphen);
                ruleDataTd.append(ruleDataInput2);

                if (rule.op === 'bt') {
                    ruleDataInputHyphen.css('display', '');
                    $(ruleDataInput2).css('display', '');
                } else {
                    ruleDataInputHyphen.css('display', 'none');
                    $(ruleDataInput2).css('display', 'none');
                }

                $(ruleDataInput)
    		    .addClass("input-elm")
    		    .bind('change', function () {
    		        rule.data = $(ruleDataInput).val() + ($(ruleDataInput2).css('display') == 'none' ? '' : ('&' + $(ruleDataInput2).val()));
    		        that.onchange(); // signals that the filter has changed
    		    });
                $(ruleDataInput2)
    		    .addClass("input-elm")
			...
			defaultFilters = $t.p.postData[p.sFilter];
                ...
                function showFilter() {
                    ...
                    if (showFrm) {
                        ...
                        $("#" + fid).jqFilter('resetFilter'); // maybe poor performance
