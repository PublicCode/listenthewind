...
_html2canvas.Parse = function (images, options) {
  ...
  function renderFormValue (el, bounds, stack){
    ...
    if (!textValue) {
        textValue = el.placeholder == undefined ? "" : el.placeholder;
    }
    ...
    switch(element.nodeName){
      ...
      case "INPUT":
        ...
        // todo add support for placeholder attribute for browsers which support it
          if (/^(text|url|email|submit|button|reset)$/.test(element.type) && ((element.value == "" ? " " : element.value) || element.placeholder).length > 0) {
