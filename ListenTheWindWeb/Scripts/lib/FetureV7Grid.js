...
var FetureV7Grid = {
    ...
    showGrid: function (fullSearchText) {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                ...
                $("#" + divName).html("<img src='/Content/images/LoadingContent.gif'/>");
                //                $("#tdfooter > img").empty();
                //                $("#tdfooter").append("<img src='/Content/images/LoadingContent.gif'/>");
            },
    ...
    parseXml: function (mydata, inCol, divName, fullSearchText) {
        ...
        mainTable.className = "DWQ_Results_table";
                ...
                if (this.inCol[i].isHidden) {
                    ...
                    if (this.inCol[i].columnName == this.sortName) {
                        if (this.sortOrder == "desc") {
                            ...
                            $(mainTh).append("<a href='#' class='gridLink' onclick='FetureV7Grid.sortColumn(\"" + this.inCol[i].columnName + "\",$(this))'>" + this.inCol[i]["disName_" + this.lang] + "&nbsp;&nbsp;</a>");
            ...
            for (i = 0; i < inCol.length; i++) {
                ...
                if (inCol[i].disType == "a") {
                    ...
                    if (inCol[i].isSpecial == "true") {
                        ...
                        $(textTd).append("<a href='#' onclick=\"" + mydata.rows[j]["strOnclick"] + "\"></a>");
                    }
                    else {
                        ...
                        if (inCol[i].gridUsedFor != undefined && inCol[i].gridUsedFor != "") {
                            ...
                            if (inCol[i].class != undefined && inCol[i].class != "") {
                                ...
                                $(textTd).append("<a href='#/" + inCol[i].gridUsedFor + "/" + dxd + "' class='" + inCol[i].class + "' onclick='javascript:FetureV7Grid.processFunction($(this)," + inCol[i].onclickFunction + ",\"" + inCol[i].linkparam + "\");'></a>");
                            }
                            else {
                                $(textTd).append("<a href='#/" + inCol[i].gridUsedFor + "/" + dxd + "' onclick='javascript:FetureV7Grid.processFunction($(this)," + inCol[i].onclickFunction + ",\"" + inCol[i].linkparam + "\");'></a>");
                            }
                        }
                        else {
                            if (inCol[i].class != undefined && inCol[i].class != "") {
                                $(textTd).append("<a href='#' class='" + inCol[i].class + "' onclick='javascript:FetureV7Grid.processFunction($(this)," + inCol[i].onclickFunction + ",\"" + inCol[i].linkparam + "\");'></a>");
                            }
                            else {
                                $(textTd).append("<a href='#' onclick='javascript:FetureV7Grid.processFunction($(this)," + inCol[i].onclickFunction + ",\"" + inCol[i].linkparam + "\");'></a>");
                            }
                        }
                    }
        ...
        if (this.currentPageIndex == 1)
            ...
            $(pageTd).append("<a class='footerlink' onclick='javascript:FetureV7Grid.couputePage(\"first\")'>" + eval("this.multiLanguage.firstPage_" + this.lang) + "</a>");
        $(pageTd).append("<span>&nbsp;&nbsp;</span>");
        if (this.currentPageIndex == 1)
            $(pageTd).append(eval("this.multiLanguage.prePage_" + this.lang));
        else
            $(pageTd).append("<a class='footerlink' onclick='javascript:FetureV7Grid.couputePage(\"up\")'>" + eval("this.multiLanguage.prePage_" + this.lang) + "</a>");
        $(pageTd).append("<span>&nbsp;&nbsp;</span>");
        if (this.currentPageIndex == tolpagesize)
            $(pageTd).append(eval("this.multiLanguage.nextPage_" + this.lang));
        else
            $(pageTd).append("<a class='footerlink' onclick='javascript:FetureV7Grid.couputePage(\"down\")'>" + eval("this.multiLanguage.nextPage_" + this.lang) + "</a>");
        $(pageTd).append("<span>&nbsp;&nbsp;</span>");
        if (this.currentPageIndex == tolpagesize)
            $(pageTd).append(eval("this.multiLanguage.lastPage_" + this.lang));
        else
            $(pageTd).append("<a class='footerlink' onclick='javascript:FetureV7Grid.couputePage(\"last\")'>" + eval("this.multiLanguage.lastPage_" + this.lang) + "</a>");
        $(pageTd).append("<span>&nbsp;&nbsp;</span>");
        ...
        var divHeight;
        //divHeight = $(mainTable).innerHeight();
        //if (divHeight < 252) {
        //    divHeight = 252;
        //}
        //divHeight += 20;
        ...
        var divWidth = $(window).width() - 240; // $(windows).screen.width() - 220px;
        //mainDiv.style.cssText = "overflow-x:auto;overflow-y:auto;width:" + eval(document.documentElement.clientWidth - 500) + "px; height:" + divHeight + "px";
        if (this.reduceWidth)
        {
        ...
        t2v_bussiness.lighthouse.ResizeFutureV7Grid();

        /*
        if (window.selectedChkAll.indexOf("," + $("#currentPage").val() + ",") > -1) {
