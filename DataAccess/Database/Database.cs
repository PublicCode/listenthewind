...
using T2VSoft.EnterpriseLibrary.Properties;
using System.Data;
...
namespace DataAccess.Database
{
    public class Database
    {
        ...
        public void LoadDataSet(DbCommand command, DataSet dataSet, string tableName, DbTransaction tanks)
        {
            LoadDataSet(command, dataSet, new string[] { tableName }, tanks);
        }
        public void LoadDataSet(DbCommand command, DataSet dataSet, string[] tableNames, DbTransaction tanks)
        {

            try
            {
                PrepareCommand(command, tanks);
